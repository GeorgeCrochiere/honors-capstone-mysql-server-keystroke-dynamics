...
public:
    ...
    KeystrokeDynamics()
    {
        std::string temp[] = {"select", "update", "from", "where", "delete", "create", "use", "describe", "table", "database", "and"};
    ...
    std::string getTestedWords(int index)
    {
        return testedWords[index];
    }

    int getNumTestedWords()
    {
        return 11;
    }
    ...
    int checkWordInList(std::string word)
    {
        ...
        for (int i = 0; i < testedWords->length(); i++)
        {
            if (testedWords[i] == testWord)
            {
                return i;
            }
        }
...
const std::string KeystrokeDynamics::MEASURED_DATA_FP = "/tmp/compiledKeyData.txt";
