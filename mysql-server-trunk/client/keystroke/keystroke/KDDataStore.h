...
public:
    ...
    {
        return ((kdLetterData[baseLetter].useLetter == true) && (kdLetterData[baseLetter].useSecondLetter[(int)letterAfter - 97] == true));
    }
    ...
    int addEntryBetween(char baseLetter, char letterAfter, long DDTime, long UDTime)
    {
    ...
    void printAllPresent()
    {
        for (int i = 97; i <= 122; i++)
        {
            ...
            for (int j = 0; j < 26; j++)
            {
                std::cout << "(" << kdLetterData[(char)i].useSecondLetter[j] << "," << kdLetterData[(char)i].countSecond[j] << ")";
            }
            std::cout << "\n";
        }
    }
    ...
    double getTestData(std::string type, char firstLetter, char secondLetter)
    {
        if (type == "DU")
        {
            return ((double)(this->kdLetterData[firstLetter].DUSummation)) / ((double)(this->kdLetterData[firstLetter].countHold));
        }
        else if (type == "DD")
        {
            return ((double)(this->kdLetterData[firstLetter].DDSummation[((int)secondLetter) - 97])) / ((double)(this->kdLetterData[firstLetter].countSecond[((int)secondLetter) - 97]));
        }
        else if (type == "UD")
        {
            return ((double)(this->kdLetterData[firstLetter].UDSummation[((int)secondLetter) - 97])) / ((double)(this->kdLetterData[firstLetter].countSecond[((int)secondLetter) - 97]));
        }
        else
        {
            return -1;
