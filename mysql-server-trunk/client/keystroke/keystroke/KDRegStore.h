...
private:
    ...
    // As variance and covariance are based on both what keystrokes were added and which ones are needed, calc
...
public:
    KDRegStore(std::string input = "")
    {
        ...
        if (input.length() != 0)
        {
            ...
            for (int i = 0; i < numElements; i++)
            {
                ...
                std::string dataLine = input.substr(indexStart + 1, (indexEnd - indexStart) - 1);
    ...
    void addEntry(KDDataStore data)
    {
        this->registationData.push_back(data);
    }

    int numItems()
    {
        return this->registationData.size();
    }

    KDDataStore getItem(int index)
    {
        return this->registationData.at(index);
    }

    // Get means
    long getMeanDU(char letter)
    {
        return this->meanHold[((int)(letter)) - 97];
    }
    ...
    void calcMeans()
    {
        ...
        for (int i = 0; i < 26; i++)
        {
            ...
            for (int j = 0; j < this->registationData.size(); j++)
            {
                if (this->registationData.at(j).getKDData((char)(97 + i)).useLetter == true)
                {
                    tempHold += (this->registationData.at(j).getKDData((char)(97 + i)).DUSummation) / (this->registationData.at(j).getKDData((char)(97 + i)).countHold);
                    // For each second letter
                    for (int k = 0; k < 26; k++)
                    {
                        if (this->registationData.at(j).getKDData((char)(97 + i)).useSecondLetter[k] == true)
                        {
                            tempDD[k] += (this->registationData.at(j).getKDData((char)(97 + i)).DDSummation[k]) / (this->registationData.at(j).getKDData((char)(97 + i)).countSecond[k]);
                            tempUD[k] += (this->registationData.at(j).getKDData((char)(97 + i)).UDSummation[k]) / (this->registationData.at(j).getKDData((char)(97 + i)).countSecond[k]);
                        }
                    }
                }
            }
    ...
    void calcVariances()
    {
        ...
        for (int i = 0; i < 26; i++)
        {
            ...
            for (int j = 0; j < this->registationData.size(); j++)
            {
                if (this->registationData.at(j).getKDData((char)(97 + i)).useLetter == true)
                {
                    tempHold += (((this->registationData.at(j).getKDData((char)(97 + i)).countHold) / (this->registationData.at(j).getKDData((char)(97 + i)).countHold)) - (this->meanHold[i])) ^ 2;
                    // For each second letter
                    for (int k = 0; k < 26; k++)
                    {
                        if (this->registationData.at(j).getKDData((char)(97 + i)).useSecondLetter[k] == true)
                        {
                            tempDD[k] += (((this->registationData.at(j).getKDData((char)(97 + i)).DDSummation[k]) / (this->registationData.at(j).getKDData((char)(97 + i)).countSecond[k])) - (this->meanDD[i][j])) ^ 2;
                            tempUD[k] += (((this->registationData.at(j).getKDData((char)(97 + i)).UDSummation[k]) / (this->registationData.at(j).getKDData((char)(97 + i)).countSecond[k])) - (this->meanUD[i][j])) ^ 2;
                        }
                    }
                }
            }
    ...
    double getCovariance(int type, char letter1, char letter1After = NULL, int type2, char letter2, char letter2After = NULL)
    {
        if (type != 1 && letter1After == NULL)
        {
            return -1;
        }
        if (type2 != 1 && letter2After == NULL)
        {
            return -1;
        }

        double covarianceVal = 0;
        for (int i = 0; i < this->registationData.size(); i++)
        {
            double val1 = 0;
            double val2 = 0;
            ...
            case KDRegStore::DOWN_UP:
                val1 = (((this->registationData.at(i).getKDData(letter1).DUSummation) / (this->registationData.at(i).getKDData(letter1).countHold)) - (this->meanHold[i]));
                break;
            case KDRegStore::DOWN_DOWN:
                val1 = (((this->registationData.at(i).getKDData(letter1).DDSummation[((int)letter1After) - 97]) / (this->registationData.at(i).getKDData(letter1).countSecond[i])) - (this->meanDD[((int)letter1) - 97][((int)letter1After) - 97]));
                break;
            case KDRegStore::UP_DOWN:
                val1 = (((this->registationData.at(i).getKDData(letter1).UDSummation[((int)letter1After) - 97]) / (this->registationData.at(i).getKDData(letter1).countSecond[i])) - (this->meanUD[((int)letter1) - 97][((int)letter1After) - 97]));
                break;
            default:
                val1 = 0;
                break;
            ...
            {
            case KDRegStore::DOWN_UP:
                val2 = (((this->registationData.at(i).getKDData(letter2).DUSummation) / (this->registationData.at(i).getKDData(letter2).countHold)) - (this->meanHold[i]));
                break;
            case KDRegStore::DOWN_DOWN:
                val2 = (((this->registationData.at(i).getKDData(letter2).DDSummation[((int)letter2After) - 97]) / (this->registationData.at(i).getKDData(letter2).countSecond[i])) - (this->meanDD[((int)letter2) - 97][((int)letter2After) - 97]));
                break;
            case KDRegStore::UP_DOWN:
                val2 = (((this->registationData.at(i).getKDData(letter2).UDSummation[((int)letter2After) - 97]) / (this->registationData.at(i).getKDData(letter2).countSecond[i])) - (this->meanUD[((int)letter2) - 97][((int)letter2After) - 97]));
                break;
            default:
                val2 = 0;
                break;
    ...
    std::string toString()
    {
        ...
        for (int i = 0; i < this->registationData.size(); i++)
        {
            ...
            if (i < this->registationData.size() - 1)
            {
                stringBuilder << ",";
            }
        }
    ...
    bool testUserEntry(KDDataStore testData, double compareVal)
    {
        ...
        for (int i = 0; i < capturedFeatures; i++)
        {
            double meanVal = getMean(type.at(i), firstLetter.at(i), secondLetter.at(i));
            double testVal = testData.getTestData(type.at(i), firstLetter.at(i), secondLetter.at(i));
            leftVector[i] = (meanVal - testVal);
            rightVector[i] = (meanVal - testVal);
            covarianceMatrix[i][i] = getVariance(type.at(i), firstLetter.at(i), secondLetter.at(i));
        ...
        for (int i = 0; i < capturedFeatures; i++)
        {
            for (int j = 0; i < capturedFeatures; j++)
            {
                if (i != j)
                {
                    ...
                    covarianceMatrix[i][j] = getCovariance(t1, firstLetter.at(i), secondLetter.at(i), t2, firstLetter.at(j), secondLetter.at(j));
                }
            }
        }
        ...
        for (int i = 0; i < capturedFeatures; i++)
        {
            ...
            for (int j = 0; j < (capturedFeatures * 2); j++)
            {
                covarianceMatrix[i][j] = covarianceMatrix[i][j] / covarianceMatrix[i][i];
            }
            for (int j = 0; j < capturedFeatures; j++)
            {
                if (i != j)
                {
                    ...
                    {
                        covarianceMatrix[j][k] = covarianceMatrix[j][k] - (covarianceMatrix[j][j] * covarianceMatrix[i][k]);
                    }
                }
            }
        }
        ...
        for (int i = 0; i < capturedFeatures; i++)
        {
            ...
            for (int j = 0; j < capturedFeatures; j++)
            {
                covarianceMatrixInverse[i][j] = covarianceMatrix[i + capturedFeatures][j];
            }
        }
    ...
    double getMean(std::string type, char firstLetter, char secondLetter)
    {
        ...
        {
            return (double)this->meanDD[(int)(firstLetter - 97)][(int)(secondLetter - 97)];
        }
        else if (type == "UD")
        {
            return (double)this->meanUD[(int)(firstLetter - 97)][(int)(secondLetter - 97)];
        }
        else
        {
            return -1;
    ...
    double getVariance(std::string type, char firstLetter, char secondLetter)
    {
        ...
        {
            return this->varianceDD[(int)(firstLetter - 97)][(int)(secondLetter - 97)];
        }
        else if (type == "UD")
        {
            return this->varianceUD[(int)(firstLetter - 97)][(int)(secondLetter - 97)];
        }
        else
        {
            return -1;
